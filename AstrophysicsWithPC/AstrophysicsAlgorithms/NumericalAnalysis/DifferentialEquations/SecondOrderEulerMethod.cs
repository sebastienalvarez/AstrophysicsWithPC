 ...
 *         différentielles du 1er ordre chainées par la méthode de Euler.
 * 
 ****************************************************************************************************************************************/
...
{
    public class SecondOrderEulerMethod : SecondOrderDifferentialEquation
    {
        // CONSTRUCTEUR
        /// <summary>
        ...
        public SecondOrderEulerMethod(Func<double, double, double, double>[] a_equations) : base(a_equations)
        {
        }
        ...
        public override double[] ComputeForGivenX(double a_x, bool a_isIterationDetailsAsked = false)
        {
            if (CheckIfComputationPossible(a_x))
            {
                ...
                while (values[0] < a_x)
                {
                    ...
                    values[0] += increment;
                    values[1] += increment * Equations[0](previousX, previousY, previousZ);
                    values[2] += increment * Equations[1](previousX, previousY, previousZ);
